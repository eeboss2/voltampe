				...
				<div className="col-xl-4 col-lg-6" key={id}>
					<div className="blog__one-item">
						<div className="blog__one-item-image">
							<Link href={`/news/${data.id}`}><img src={data.image.src} alt="blog" /></Link>
						</div>
						<div className="blog__one-item-content">
							...
							<h6><Link href={`/news/${data.id}`}>{data.title}</Link></h6>
							<Link className="simple-btn" href={`/news/${data.id}`}>Read More<i className="fa-sharp fa-regular fa-arrow-up-right"></i></Link>
						</div>
					</div>
				</div>
            ))}            
